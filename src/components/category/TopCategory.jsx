...
const TopCategory = ({ isMobile = false,closeDrawer }) => {
  ...
  useEffect(() => {
    ...
    const pcid = params.get("pcid") ? atob(params.get("pcid")) : null;
    const pscid = params.get("pscid") ? atob(params.get("pscid")) : null;

    if (pcid) {
      setSelectedCategory(pcid); // Set the selected category
      FetchSubcategories(pcid).then(() => {
        if (pscid) {
          // Optionally, scroll to or highlight the subcategory
          console.log("Navigate to subcategory:", pscid);
        }
      });
    }
  }, [location.search]);
